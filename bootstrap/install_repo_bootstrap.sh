...
# Note: ticket-ID patterns (e.g. ABC-123) are INTENTIONALLY not blocked in
# commit messages — CLAUDE.md §14 explicitly allows `Refs: ABC-123` trailers
# when the repo's CONTRIBUTING template requires them. Ticket IDs ARE blocked
# in code comments (post-edit-policy-check.sh and the pre-commit hook below).
...
while read -r local_ref local_sha remote_ref remote_sha; do
  ...
  if git log --format='%B' "$range" | grep -Eiq '(co-authored-by:.*(claude|anthropic|gpt|copilot)|generated with (claude|ai|gpt)|claude code|ai-generated|generated by ai|🤖|🧠)'; then
